...
public class ShopPanelView : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        {
            Debug.LogWarning("[ShopPanelView] Dependencies not injected yet.");
            return;
        }
    ...
    private void ItemOnPurchased(ItemType type)
    {
        ...
        {
            Debug.LogWarning($"No ShopItemEntry found for type {type}");
            return;
        }
        ...
        if (player == null)
        {
            Debug.LogWarning("[ShopPanelView] Player not spawned yet");
            return;
        }
